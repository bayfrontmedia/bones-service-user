...
- `user.login`: Executes when a user is logged in using the [login](userservice-class.md#login) method.
  The user ID and expiration timestamp are passed as parameters.
...
- `user.refresh`: Executes when a user session is successfully refreshed using
  the [refresh](userservice-class.md#refresh) method.
  The refresh token and a boolean value representing whether the token came from a cookie (refresh)
...
- `app.http`: The user session is started with a priority of `1`.